<x-jet-action-section>
    ...
    <x-slot name="content">
        ...
        <div class="d-flex mt-3">
            <x-jet-button wire:click="confirmLogout" wire:loading.attr="disabled" class="bg-danger co-bla">
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                ...
                <div class="mt-3 w-md-75" x-data="{}" x-on:confirming-logout-other-browser-sessions.window="setTimeout(() => $refs.password.focus(), 250)">
                    <x-jet-input type="password" placeholder="{{ __('Ingrese su Contraseña') }}"
                                 x-ref="password"
            ...
            <x-slot name="footer">
                <x-jet-secondary-button  class="bg-c-se co-bla" wire:click="$toggle('confirmingLogout')" wire:loading.attr="disabled">
                    {{ __('Cancelar') }}
                </x-jet-secondary-button>

                <x-jet-button class="ms-2 bg-c-pri" wire:click="logoutOtherBrowserSessions" wire:loading.attr="disabled">
                    <div wire:loading wire:target="logoutOtherBrowserSessions" class="spinner-border spinner-border-sm" role="status">
                        <span class="visually-hidden">Loading...</span>
                    </div>

                    {{ __('Salir de otras sesiones del navegador') }}
                </x-jet-button>
            </x-slot>
        </x-jet-dialog-modal>
    </x-slot>
